...
public:
    ...
    c_mrst99function() {}
    c_mrst99function(int mode) {
        initialise(mode);
    }
    c_mrst99function&  operator=(const c_mrst99function& a)
    {
        for (int i=0; i<nx+1; i++) xx[i]=a.xx[i];
        for (int i=0; i<nq+1; i++) qq[i]=a.qq[i];
        for (int i=0; i<np+1; i++) n0[i]=a.n0[i];
        for (int iq=0; iq<nq+1; iq++)
            for (int ix=0; ix<nx+1; ix++)
                for (int ip=0; ip<np+1; ip++) f[ip][ix][iq]=a.f[ip][ix][iq];
        return *this;
    }
