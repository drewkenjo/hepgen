...
#include "CLHEP_EMBEDDED/Random/Random/DoubConv.hh"
#include <string.h>	// for strcmp
#include <cmath>	// for std::log()

namespace CLHEP {

std::string RandGauss::name() const {
    return "RandGauss";
}
HepRandomEngine & RandGauss::engine() {
    return *localEngine;
}
...
std::istream & RandGauss::get ( std::istream & is ) {
    ...
    if (possibleKeywordInput(is, "Uvec", c1)) {
        std::vector<unsigned long> t(2);
        is >> defaultMean >> t[0] >> t[1];
        defaultMean = DoubConv::longs2double(t);
        is >> defaultStdDev>>t[0]>>t[1];
        defaultStdDev = DoubConv::longs2double(t);
        std::string ng;
        is >> ng;
        set = false;
#ifdef TRACE_IO
...
#endif
        if (ng == "nextGauss") {
            is >> nextGauss >> t[0] >> t[1];
            nextGauss = DoubConv::longs2double(t);
#ifdef TRACE_IO
