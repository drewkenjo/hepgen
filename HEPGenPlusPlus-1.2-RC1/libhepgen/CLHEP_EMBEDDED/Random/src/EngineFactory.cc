...
#include "CLHEP_EMBEDDED/Random/Random/EngineFactory.h"
#include "CLHEP_EMBEDDED/Random/Random/JamesRandom.h"
#include "CLHEP_EMBEDDED/Random/Random/Ranlux64Engine.h"
#include "CLHEP_EMBEDDED/Random/Random/RanluxEngine.h"
#include "CLHEP_EMBEDDED/Random/Random/engineIDulong.h"
#include <iostream>
#include <string>
...
HepRandomEngine* EngineFactory::newEngine(std::istream& is) {
    ...
    std::string tag;
    is >> tag;
    eptr = makeAnEngine <HepJamesRandom>  (tag, is);
    if (eptr) return eptr;
    eptr = makeAnEngine <Ranlux64Engine>  (tag, is);
    if (eptr) return eptr;
    eptr = makeAnEngine <RanluxEngine>    (tag, is);
    if (eptr) return eptr;
...
EngineFactory::newEngine(std::vector<unsigned long> const & v) {
    HepRandomEngine* eptr;
    eptr = makeAnEngine <HepJamesRandom>  (v);
    if (eptr) return eptr;
    eptr = makeAnEngine <Ranlux64Engine>  (v);
    if (eptr) return eptr;
    eptr = makeAnEngine <RanluxEngine>    (v);
    if (eptr) return eptr;
    std::cerr <<
